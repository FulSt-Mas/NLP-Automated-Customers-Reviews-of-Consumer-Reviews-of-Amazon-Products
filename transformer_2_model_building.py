"""
STEP 2.1: Transformer Model Selection (Base Model + Fine-Tuning)
Purpose:
•   Load preprocessed reviews (Step 1 output)
•   Use a pre-trained Transformer (DistilBERT) from HuggingFace
•   Run zero-shot inference (no fine-tuning) or fine-tune the model
...
class TransformerModelEvaluator:
    """
    A class to evaluate and optionally fine-tune a pre-trained Transformer model
    on sentiment classification tasks using preprocessed customer review data.
    """
    ...
    def load_model(self, num_labels=None):
        """Load tokenizer and pre-trained model, optionally for fine-tuning."""
    ...
    # ----------------------------------------------------------
    def load_data(self):
        """Load and prepare preprocessed dataset."""
    ...
    def evaluate(self, df: pd.DataFrame, fine_tune: bool = False):
        ...
        If fine_tune is True, split data, train, and evaluate the fine-tuned model.
        """
        if fine_tune:
    ...
    def _zero_shot_evaluate(self, df: pd.DataFrame):
        """Evaluate model performance on the dataset (zero-shot)."""
    ...
    def _fine_tune_and_evaluate(self, df: pd.DataFrame):
        ...
        report = classification_report(y_test, preds, target_names=["negative", "neutral", "positive"], zero_division=0)
        logger.info(f"✅ Fine-tuned model accuracy: {acc:.3f}")
        logger.info("\n" + report)
        self._save_results(acc, report, "fine_tuned_model_results.txt")
        self._save_model_and_tokenizer("fine_tuned_model_distilbert")
        self._save_model_as_joblib_and_pickle("fine_tuned_model_distilbert")
    ...
    def _save_model_as_joblib_and_pickle(self, model_dir):
        """Save model as joblib and pickle for flexibility."""
        model_save_path = os.path.join(OUT_DIR, model_dir)
...
if __name__ == "__main__":
    ...
    # To run fine-tuning and evaluation:
    # evaluator.load_model(num_labels=
